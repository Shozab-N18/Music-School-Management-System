...
class GetAllTransactionHistoryTestCase(TestCase):
    ...
    # this test test if function that display all students's transaction history work as expected
    # related view function: get_all_transactions
    ...
    def test_transaction_history_url(self):
        self.assertEqual(self.url, '/transaction_history/')

    def test_access_transaction_history_when_log_in_as_admin(self):
    ...
    def test_access_transaction_history_when_log_in_as_director(self):
        self.client.login(username=self.director.email, password='Password123')
        response = self.client.get(self.url)                         
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'transaction_history.html')

    def test_access_transaction_history_without_being_logged_in(self):
        redirect_url = reverse_with_next('home', self.url)
        response = self.client.get(self.url, follow=True)
        self.assertRedirects(response, redirect_url, status_code=302, target_status_code=200)
        self.assertTemplateUsed(response, 'home.html')

    def test_access_transaction_history_when_log_in_as_student(self):
